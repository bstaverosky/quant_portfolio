...
asset$BTC  <- ifelse(asset$score == 0, 0,
              ifelse(asset$score == 1, 0.5,
              ifelse(asset$score == 2, 0.9,1)))
...
names(return_xts) <- "Adaptive_Beta_BTC"

weights_xts <- asset[,c("cash", "BTC")]
...
export_strategy_output(
  strategy_name = "Adaptive_Beta_BTC",
