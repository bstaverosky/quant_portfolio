...
strat_returns_weeks <- portf_return_momo_erc_brian(r_full, n_assets = 3, n_days = 120, n_days_vol = 42, momo_type = "above average", otype = "returns")
strat_wts_weeks     <- portf_return_momo_erc_brian(r_full, n_assets = 3, n_days = 120, n_days_vol = 42, momo_type = "above average", otype = "weights")

charts.PerformanceSummary(strat_returns)
charts.PerformanceSummary(strat_returns_weeks)

Return.annualized(strat_returns["2010/"])
Return.annualized(strat_returns_weeks["2010/"])

SharpeRatio.annualized(strat_returns)
SharpeRatio.annualized(strat_returns_weeks)


strat3x <- strat_returns*3
charts.PerformanceSummary(strat3x)
charts.PerformanceSummary(strat3x["2024/2024"])
Return.annualized(strat3x, scale = 252)

strat3x_weeks <- strat_returns_weeks*3
charts.PerformanceSummary(strat3x)
charts.PerformanceSummary(strat3x["2024/2024"])
Return.annualized(strat3x, scale = 252)

charts.PerformanceSummary(merge(strat3x,strat3x_weeks))

Return.annualized(strat3x)
Return.annualized(strat3x["/2010"])

Return.annualized(strat3x["2010/"])
Return.annualized(strat3x_weeks["2010/"])
SharpeRatio.annualized(strat3x)
